...
#@title
'''
i) Add the correct tranform for masking tokens -> (Check which tokens are we actually masking ? are these only notes tokens or duration/instruments tokens too?)
ii) Can we use same databunch for training : as only data transform is changed 
iii) Generator class should also be modified for remixing
iv) Work on multitask learner for prediction 
v) Note we have to handle everything taking in account the instrument token 
'''
...
def mask_lm_tfm_default(b, vocab, mask_p=0.3):
    return mask_tfm_custom(b, mask_range=vocab.npenc_range, mask_idx=vocab.mask_idx, pad_idx=vocab.pad_idx, mask_p=mask_p)

def mask_lm_tfm_pitchdur(b, vocab, mask_p=0.9):
    mask_range = vocab.dur_range if np.random.rand() < 0.5 else vocab.note_range
    return mask_tfm_custom(b, mask_range=mask_range, mask_idx=vocab.mask_idx, pad_idx=vocab.pad_idx, mask_p=mask_p)    
...
def mask_lm_tfm(b, mask_range, mask_idx, pad_idx, mask_p):
    ...
    if(len(x.shape) == 3):
      ...
      x_dict = { 
          'msk': { 'x': x_msk, 'pos': None },
          'lm': { 'x': x_lm, 'pos': None }
      }
      y_dict = { 'msk': y_msk, 'lm': y_lm }
    return x_dict, y_dict

def mask_tfm_custom(b, mask_range, mask_idx, pad_idx, mask_p):
    x,y = b
...
def multitask_model_learner(data:DataBunch, config:dict=None, drop_mult:float=1., 
    ...
    metrics = [AverageMultiMetric(partial(m, pad_idx=vocab.pad_idx)) for m in [mask_acc]]
...
class MultitaskLearner(Learner):
    ...
    def predict_mask(self, masked_item:MusicItem,
        ...
        for midx in mask_idxs:
            ...
            # We changed ths to depend on instrument token in nw prediction
            # Here we are iterating on mask_idxs so there is a good chance that prev_idxs are all durations or all note tokens 
            ...
            # Filter
            filter_value = -float('Inf')
            special_idxs = [vocab.bos_idx, vocab.sep_idx, vocab.stoi[IN], vocab.stoi[EOS], vocab.stoi[ELECTRONIC], vocab.stoi[FOLK], vocab.stoi[FUNK], vocab.stoi[JAZZ], vocab.stoi[POP], vocab.stoi[ROCK]]
